...
- **User Profile (`ProfileScreen`)**: Top header with settings shortcut, 52px circular avatar featuring bundled 3D clay-style Black character avatar (`assets/avatar.png`) with interactive Camera Edit badge (`avatarCameraBadge`), **Avatar Photo Update Modal** (`AvatarUpdateModal`) supporting native camera capture (`ImagePicker.launchCameraAsync`) and photo library picking (`ImagePicker.launchImageLibraryAsync`) with runtime permission prompts and local `AsyncStorage` caching, Wallet Quick-Access card with interactive privacy eye toggle (`👁` / `👁‍🗨`) to hide/show balances, My Orders status shortcuts (`To Pay`, `To Ship`, `To Receive`, `To Review`, `Refund`), My Tools shortcuts (`Followed Stores`, `Favorites`, `Footprints`, `Shipping Address`), recommended product feed, and `FlatList` with native `RefreshControl`.
- **Followed Stores (`FollowedStoresScreen`)**: Dedicated screen displaying all followed stores with store avatars, verification badges, ratings, follower count, location, follow/unfollow action, and a horizontal carousel of each store's latest products with 1-tap cart addition.
- **Favorites / Wishlist (`FavoritesScreen`)**: Dedicated screen displaying all liked products in a 2-column grid, hooked into `useFavoritesStore` and love heart triggers across `ProductCard` and `ProductDetailScreen`, with instant Add to Cart and Clear All actions.
- **Browsing Footprint (`FootprintScreen`)**: Chronological browsing history logging up to 50 recently viewed items via `useFootprintStore`, with formatted timestamps (`formatDate`), individual removal, and Clear All action.
- **Refunds & Escrow Disputes (`RefundsScreen`)**: Dedicated screen with segmented tab controls:
  - **In Progress / Under Review**: Active escrow disputes with dispute reason, evidence photos, frozen escrow status, and 48-hour staff arbitration timeline.
