...
| 2.9 | September 4, 2026 | Architecture & Engineering Team | End-to-End API Integration, Direct Native Phone Dialer & Transporter Camera Sensor Release (v2.9): (1) Direct Native Phone Dialer Integration (`Linking.openURL('tel:...')`) across all merchant, buyer, transporter, and store contact call buttons and phone info rows. (2) Live Hardware Camera Sensor QR & Barcode Scanner (`CameraView` from `expo-camera`) in Transporter App Bar (`TransporterJobsScreen.tsx` & `TransporterActiveTripScreen.tsx`) with real-time sensor detection, torch flashlight, and laser reticle. (3) Seller Store Profile & Branding Architecture (`EditStoreProfileScreen.tsx` & `SellerProfileScreen.tsx`) connected to `POST /api/v1/seller/store/profile` backend endpoint. (4) 5-Digit Handover Security PIN Verification System (`orders.pickup_pin`). (5) Delivery Fee Restructuring & Personal Courier / Self-Pickup Workflow (`BuyerCartScreen.tsx` & `CheckoutPaymentScreen.tsx`). |
