...
| 2.6 | September 3, 2026 | Architecture & Engineering Team | Persona Switcher ACL Security Guard & Per-Account Auth Persistence Release (v2.6): (1) Registered `switch_staff_personas` ACL permission flag restricted by default to Super Admin (`SUPER_ADMIN` / Level 5 Clearance). (2) Persona switcher dropdown section in `Header.tsx` completely removed for all standard staff roles, and rendered only when explicitly granted via ACL. (3) Per-Staff Account Authentication Matching & Session Hydration in `staffAuthStore.ts` & `AuthPage.tsx` with Quick-Select Dropdown for all corporate staff members. (4) Immutable security audit log capture for all login, logout, persona switch, staff CRUD, and operational actions. |
